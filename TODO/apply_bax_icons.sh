...
for sz in "${sizes[@]}"; do
  convert -background none -resize ${sz}x${sz} "$LOGO_MAIN" "$SRC_DIR/bax_icon_${sz}.png"
  convert -background none -resize ${sz}x${sz} "$LOGO_WHITE" "$SRC_DIR/bax_white_icon_${sz}.png"
done

# 2. Generate .ico (Windows desktop icon) using ImageMagick only
# (multi-size: 16,32,48,64,128,256)
convert "$SRC_DIR/bax_icon_16.png" \
...
for d in "${WIN32_DIRS[@]}"; do
  ...
  for sz in 100 125 150 175 200 225 250; do
    convert "$SRC_DIR/bax_icon_256.png" -resize $((sz*2))x$((sz*2)) "$ROOT_DIR/$d/inno-big-${sz}.bmp"
  done
  # Small bmps (using 100,125,150,175,200,225,250)
  for sz in 100 125 150 175 200 225 250; do
    convert "$SRC_DIR/bax_icon_70.png" -resize $((sz/2))x$((sz/2)) "$ROOT_DIR/$d/inno-small-${sz}.bmp"
  done
  ...
  for icon in bower c config cpp csharp css default go html jade java javascript json less markdown php powershell python react ruby sass shell sql typescript vue xml yaml; do
    convert "$SRC_DIR/bax_white_icon_128.png" "$ROOT_DIR/$d/${icon}.ico"
  done
done
...
for d in "${DARWIN_DIRS[@]}"; do
  ...
  for icon in bower c config cpp csharp css default go html jade java javascript json less markdown php powershell python react ruby sass shell sql typescript vue xml yaml; do
    convert "$SRC_DIR/bax_white_icon_128.png" -resize 128x128 "$ROOT_DIR/$d/${icon}.png"
  done
done
...
for d in "${LINUX_DIRS[@]}"; do
  ...
  convert "$SRC_DIR/bax_icon_512.png" "$ROOT_DIR/$d/rpm/code.xpm"
done
